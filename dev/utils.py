...
from anywall_app.service import (
    createMockedWindowObject, createMockedReqWindowObject, 
    createMockedBackupWindowObject, createMockedStateObject, 
    createMockedDeltaObject, createMockedApiCallObject, 
    read_windows, read_requested_windows
)
...
def getReceivedApiCalls(last_api_call_dt_creation):
    ...
    try:
        api_calls = list(django_api_calls.objects.filter(
...
def callAlarmExpired():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            logger.warning(f"Failed to check expired alarms. Status code: {response.status_code}")
            return None
    except Exception as e:
...
def callAlarmClear():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            logger.warning(f"Failed to clear alarms. Status code: {response.status_code}")
            return None
    except Exception as e:
...
def isAlarmExpired(timeout):
    ...
    try:
        ...
        if current_time > timeout_time:
            logger.debug(f"Alarm expired. Timeout: {timeout_time}, Current: {current_time}")
            return True
        return False
    except Exception as e:
